...
현재 소스 코드는 앱 기반과 Claude·Codex 상태 관찰, CLI 연결, 대표 작업 이동 및 첫 펫·카드 화면까지 구현한 개발 버전입니다. 완료 확인, 전체 설정 화면, 스프라이트 펫과 서명된 업데이트는 아직 구현 중입니다.
...
현재 기본 펫은 프로젝트 코드로 그리는 원본 벡터 펫입니다. codex-pets 호환 스프라이트의 설치와 애니메이션은 후속 구현 범위입니다.
